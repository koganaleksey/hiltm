...
function ftpDeploy() {
  ...
  const globs = ["dist/**/*"];
  return src(globs, { buffer: false }).pipe(conn.dest("/hiltm/public_html/"));
}
...
function copyActions() {
  log(
    chalk.red.bold(
      "---------------COPYING ACTIONS INTO DIST FOLDER---------------"
    )
  );
  return src(["src/actions/*"])
    .pipe(dest("dist/actions"))
    .pipe(browserSync.stream());
}

function copyPHP() {
  log(
    chalk.red.bold("---------------COPYING PHP INTO DIST FOLDER---------------")
  );
...
function copyFavicon() {
  log(
    chalk.red.bold(
      "---------------COPYING FAVICON ICO INTO DIST FOLDER---------------"
    )
  );
...
function jsVendor() {
  log(
    chalk.red.bold(
      "---------------COPY JAVASCRIPT VENDOR FILES INTO DIST---------------"
    )
  );
...
function cssVendor() {
  log(
    chalk.red.bold(
      "---------------COPY CSS VENDOR FILES INTO DIST---------------"
    )
  );
...
function generateDocs() {
  ...
  return src(["dist/**/*"]).pipe(dest("docs")).pipe(browserSync.stream());
}
